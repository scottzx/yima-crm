...
(DATABASE_URL ? describe : describe.skip)(
  ...
  () => {
    ...
    const migration = new AgentHistoryMigrationService(dataSource, storage);
    ...
    it('copies all five tables, exact credits and archive state before changing the route', async () => {
      await migration.migrate(WORKSPACE_ID, 'workspace', false, 1);
    ...
    it('retains the write fence after a crash and resumes committed batches', async () => {
      ...
      await expect(
        migration.migrate(WORKSPACE_ID, 'workspace', false, 1),
    ...
    it('rolls back new writes and deletions rather than selecting the stale core snapshot', async () => {
      ...
      await migration.migrate(WORKSPACE_ID, 'core', false, 1);
      expect(
    ...
    it('refuses cutover while a stream is active', async () => {
      ...
      await expect(
        migration.migrate(WORKSPACE_ID, 'workspace', false),
    ...
    it('uses workspace ORM reads, archives and atomic stream claims without emitting generic events', async () => {
      await migration.migrate(WORKSPACE_ID, 'workspace', false);
      expect(
    ...
    it('keeps the fence when a copied value fails verification, then allows abort', async () => {
      ...
      await migration.abort(WORKSPACE_ID, false);
      await storage.run(WORKSPACE_ID, async ({ storage: selected }) =>
        expect(selected).toBe('core'),
      );
      expect(
    ...
    it('cleans only obsolete snapshots after retention and can still reverse-copy for rollback', async () => {
      await migration.migrate(WORKSPACE_ID, 'workspace', false);
      await expect(
        migration.cleanup(WORKSPACE_ID, false, 14),
      ...
      await migration.cleanup(WORKSPACE_ID, false, 14);
      expect(
        (
          await dataSource.query('SELECT count(*) FROM core."agentChatThread"')
        )[0].count,
      ...
      await migration.migrate(WORKSPACE_ID, 'core', false);
      expect(
        (
          await dataSource.query('SELECT count(*) FROM core."agentChatThread"')
        )[0].count,
    ...
    it('skips ineligible cleanup but rejects corrupt durable state', async () => {
      await expect(
        migration.cleanup(WORKSPACE_ID, true, 14),
      ).resolves.toBeUndefined();
      await expect(
        migration.cleanup(WORKSPACE_ID, false, 14),
      ).resolves.toBeUndefined();
      ...
      await expect(migration.cleanup(WORKSPACE_ID, true, 14)).rejects.toThrow(
    ...
    it('rejects a destination collision introduced after preflight without advancing the cursor', async () => {
      ...
      await expect(
        migration.migrate(WORKSPACE_ID, 'core', false, 1),
    ...
    it('does not allow copy to resume after an interrupted abort', async () => {
      ...
      await expect(migration.abort(WORKSPACE_ID, false)).rejects.toThrow(
        'abort interrupted',
      );
