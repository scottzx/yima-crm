...
export class AgentHistoryMigrateCommand extends ProvisionedWorkspaceCommandRunner<Options> {
  ...
  override async runOnWorkspace({
    ...
    const migrationOptions = options as Options;
    const dryRun = options.dryRun ?? false;
    if (migrationOptions.abort) {
      await this.migration.abort(workspaceId, dryRun);
      return;
    }
    if (!dryRun && !migrationOptions.routingDeployed)
      throw new Error(
    ...
    await this.migration.migrate(
      workspaceId,
      migrationOptions.target ?? 'workspace',
      false,
      migrationOptions.batchSize ?? 1000,
    );
  }
}
